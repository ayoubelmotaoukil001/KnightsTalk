...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-white leading-tight">Edit Lesson &mdash; {{ $lesson->title }}</h2>
    </x-slot>
    ...
    <div class="py-8 px-4 max-w-7xl mx-auto">
        ...
        <form id="lesson-form" method="POST" action="{{ route('admin.lessons.update', [$course, $lesson]) }}">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title" value="{{ $lesson->title }}" required class="w-full max-w-md bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 placeholder-gray-500 transition-all duration-200">
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                ...
                <div>
                    <div class="mb-4">
                        ...
                        <textarea id="move_input" rows="3" class="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 font-mono text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 placeholder-gray-500 transition-all duration-200">{{ $lesson->moves_sequence }}</textarea>
                    </div>
                    ...
                    <div id="save-section" class="hidden mt-6">
                        <button type="submit" class="bg-gradient-to-r from-emerald-500 to-emerald-700 text-white px-5 py-2.5 rounded-xl hover:from-emerald-400 hover:to-emerald-600 text-sm font-medium transition-all duration-200 shadow-lg shadow-emerald-500/20">Update Lesson</button>
                    </div>
                </div>
            </div>
        </form>
        ...
        function buildCards(movesArr, descriptions) {
            ...
            sanMoves.forEach(function (san, idx) {
                ...
                card.innerHTML = '<p class="font-bold text-sm mb-2 text-white">' + num + ' ' + san + '</p>' +
                    '<textarea id="desc-'+idx+'" rows="2" placeholder="Describe this move..." class="w-full bg-white/5 border border-white/10 text-white rounded-lg px-2 py-1 text-sm placeholder-gray-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" onclick="event.stopPropagation()">' + desc.replace(/</g,'&lt;') + '</textarea>';

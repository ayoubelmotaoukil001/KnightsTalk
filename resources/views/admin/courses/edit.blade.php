<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold leading-tight text-slate-900 dark:text-white">Edit Course: {{ $course->title }}</h2>
    </x-slot>

    <div class="mx-auto max-w-2xl px-4 py-8">

        <a href="{{ route('admin.courses.show', $course) }}" class="mb-4 inline-block text-sm font-medium text-red-600 transition-colors hover:text-red-500 dark:text-red-400 dark:hover:text-red-300">&larr; Back to Course</a>

        <form method="POST" action="{{ route('admin.courses.update', $course) }}" class="space-y-5 rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.05] dark:shadow-none">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full rounded-xl border border-slate-200/90 bg-white px-4 py-3 text-slate-900 transition-all duration-200 placeholder:text-slate-400 focus:border-red-500 focus:ring-1 focus:ring-red-500 dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-red-400 dark:focus:ring-red-400">{{ old('description', $course->description) }}</textarea>
